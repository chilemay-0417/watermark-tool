...
本文适用于 **2.2.0**。右键菜单「添加水印」会读取项目中的配置、签名和 Logo，修改后下次处理生效，无需重新安装右键操作。命令行示例在已激活项目环境的终端中运行，见 [命令与参数](usage.md#命令与参数)。
...
以下规则适用于命令行多图拼接；右键「添加水印」选中多张照片时逐张导出，每张都独立选择 Logo 和签名。
